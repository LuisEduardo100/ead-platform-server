...
export const episodesController = {
  ...
  showQuizz: async (req: Request, res: Response) => {
    ...
    try {
      const episodeWithQuizz = await episodeService.getEpisodeWithQuizz(episodeId)
      if (!episodeWithQuizz) return res.status(404).json({ message: "Função showQuiz error: Ep com quizz não encontrado" })
